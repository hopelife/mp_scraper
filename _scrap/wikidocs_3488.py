...
base_path = '../../../__references/_python/sats/_scrap/wikidocs/3488_금융 데이터 분석을 위한 파이썬 Pandas/source/'  ## 'https://wikidocs.net/book/3488'
base_url = 'https://wikidocs.net/'
url = 'https://wikidocs.net/book/3488'
defaults = dict(
    base_url = base_url,
    base_path = base_path, 
    head = {
...
#         url = f"/{url.split('(')[-1][:-1]}"
#         title = el.xpath('./span/span')[0].text.strip().replace('.', '').replace('/', '_')

#         prefix = title.split(' ')[0]
#         if prefix[-1] == '장':
#             prefix = prefix[:-1]
#         if prefix[-1] == ')':
#             prefix = f'05-{prefix[:-1]}'  ## NOTE: 5장의 절은 '01) ' 형식으로 되어 있음
